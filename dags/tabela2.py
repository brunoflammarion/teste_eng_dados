...
# define the dag 
dag = DAG('tabela2', # give the dag a name 
...
bq_task_1 = BigQueryOperator(
        ...
        task_id='process_tabela2', # task id's must be uniqe within the dag
        bql='select  marca, linha, sum(QTD_VENDA) total from [teste_boticatio.base_boticario] group by 1,2 order by 1,2', # the actual sql command we want to run on bigquery is in this file in the same folder. it is also templated
        destination_dataset_table='teste_boticatio.tabela2', # we also in this example want our target table to be lob and task specific
