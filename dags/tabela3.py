...
# define the dag 
dag = DAG('tabela3', # give the dag a name 
...
bq_task_1 = BigQueryOperator(
        ...
        task_id='process_tabela3', # task id's must be uniqe within the dag
        bql='select  marca, year(data_venda) ano, month(data_venda) mes, sum(QTD_VENDA) total from [teste_boticatio.base_boticario] group by 1,2,3 order by 1,2,3', # the actual sql command we want to run on bigquery is in this file in the same folder. it is also templated
        destination_dataset_table='teste_boticatio.tabela3', # we also in this example want our target table to be lob and task specific
